...
import oceanus.utils.GatedSplittedSRAM
...
class DataStorageSRAM(implicit val p: Parameters) extends Module with HasL2Params {
  ...
  val debugCycle = RegInit(0.U(32.W))
  debugCycle := debugCycle + 1.U
  val debugWindow = debugCycle >= 318.U && debugCycle <= 325.U
  when(debugWindow) {
    printf(p"[dstorage-sram] cycle=${debugCycle} readFire=${readFire} " +
      p"set=0x${Hexadecimal(io.req.bits.set)} way=0x${Hexadecimal(io.req.bits.way)} " +
      p"arrayRead=0x${Hexadecimal(arrayRead.data)} ioRdata=0x${Hexadecimal(dataRead.data)}\n")
  }
  ...
  assert(!(RegNext(io.en) && (io.req.asUInt =/= RegNext(io.req.asUInt))),
    "DataStorageSRAM: req fails to hold for 2 cycles")
...
class DataStorage(implicit val p: Parameters) extends Module with HasL2Params {
  ...
  val issuedReq_rr = RegNext(issuedReq_r)
  ...
  sram.io.req.bits.way := Mux(datStop_r, issuedReq_r.way, issueReq.way)
  sram.io.req.bits.set := Mux(datStop_r, issuedReq_r.set, issueReq.set)
  sram.io.req.bits.wen := Mux(datStop_r, issuedReq_r.wen, issueReq.wen)
  sram.io.wdata.data   := Mux(datStop_r, issuedReq_r.wdata, issueReq.wdata)

  val sramReqPrev = RegNext(sram.io.req.bits)
  val sramReqIdx = Cat(sram.io.req.bits.way, sram.io.req.bits.set)
  val sramReqPrevIdx = RegNext(sramReqIdx)
  ...
  when(debugWindow) {
    val reqSetMismatch = sram.io.req.bits.set =/= sramReqPrev.set
    val reqWayMismatch = sram.io.req.bits.way =/= sramReqPrev.way
    val reqWenMismatch = sram.io.req.bits.wen =/= sramReqPrev.wen
    printf(p"[l2ds-hold] cycle=${debugCycle} anyGrant=${anyGrant} datStop_r=${datStop_r} " +
      p"grantRd=${grantRd} grantAheadRd=${grantAheadRd} grantWb=${grantWb} " +
      p"winIdx=0x${Hexadecimal(winIdx)} winRd=0x${Hexadecimal(winRd)} winAheadRd=0x${Hexadecimal(winAheadRd)} winWb=0x${Hexadecimal(winWb)} " +
      p"selSet=0x${Hexadecimal(issueSel.SET)} selWay=0x${Hexadecimal(issueSel.WAY)} selWen=${grantWb} " +
      p"selDataLo=0x${Hexadecimal(issueSel.DATA(63, 0))} issueOwner=0x${Hexadecimal(issueReq.owner)} " +
      p"datReqSet=0x${Hexadecimal(issuedReq_r.set)} datReqWay=0x${Hexadecimal(issuedReq_r.way)} datReqWen=${issuedReq_r.wen} " +
      p"datReqOwner=0x${Hexadecimal(issuedReq_r.owner)} datReqDataLo=0x${Hexadecimal(issuedReq_r.wdata(63, 0))} " +
      p"sramReqIdx=0x${Hexadecimal(sramReqIdx)} prevReqIdx=0x${Hexadecimal(sramReqPrevIdx)} " +
      p"sramReqSet=0x${Hexadecimal(sram.io.req.bits.set)} sramReqWay=0x${Hexadecimal(sram.io.req.bits.way)} sramReqWen=${sram.io.req.bits.wen} " +
      p"prevReqSet=0x${Hexadecimal(sramReqPrev.set)} prevReqWay=0x${Hexadecimal(sramReqPrev.way)} prevReqWen=${sramReqPrev.wen} " +
      p"mismatch(set=${reqSetMismatch},way=${reqWayMismatch},wen=${reqWenMismatch})\n")
    printf(p"[l2ds-issued-pack] cycle=${debugCycle} issueReq=0x${Hexadecimal(issueReq.asUInt)} " +
      p"issuedReq_r=0x${Hexadecimal(issuedReq_r.asUInt)} issuedReq_rr=0x${Hexadecimal(issuedReq_rr.asUInt)}\n")
    io.fromTSHR.zipWithIndex.foreach { case (req, i) =>
      printf(p"[l2ds-tsreq] cycle=${debugCycle} idx=${i.U} rd=${req.DSBufRd} ar=${req.DSBufAheadRd} wb=${req.DSBufWb} " +
        p"set=0x${Hexadecimal(req.SET)} way=0x${Hexadecimal(req.WAY)} dataLo=0x${Hexadecimal(req.DATA(63, 0))}\n")
    }
  }
  ...
  when(debugWindow) {
    printf(p"[l2ds-pipe] cycle=${debugCycle} grantRd=${grantRd} grantAheadRd=${grantAheadRd} " +
      p"winRd=${Hexadecimal(winRd)} winAheadRd=${Hexadecimal(winAheadRd)} winIdx=${Hexadecimal(winIdx)} " +
      p"readReqFire=${readReqFire} readReqFire_r=${readReqFire_r} " +
      p"owner_r=${Hexadecimal(readReqOwner_r)} " +
      p"datStop_r=${datStop_r} datReqSet=0x${Hexadecimal(issuedReq_r.set)} datReqWay=0x${Hexadecimal(issuedReq_r.way)} " +
      p"datReqOwner=0x${Hexadecimal(issuedReq_r.owner)} datReqDataLo=0x${Hexadecimal(issuedReq_r.wdata(63, 0))} " +
      p"sramReqIdx=0x${Hexadecimal(sramReqIdx)} " +
      p"selSet=0x${Hexadecimal(issueSel.SET)} selWay=0x${Hexadecimal(issueSel.WAY)} " +
      p"sramReqSet=0x${Hexadecimal(sram.io.req.bits.set)} sramReqWay=0x${Hexadecimal(sram.io.req.bits.way)} " +
      p"sramEn=${sram.io.en} rdata=0x${Hexadecimal(sram.io.rdata.data)}\n")
  }
  ...
  for (i <- 0 until nMSHR) {
    ...
    when(rdRespHit) {
      printf(p"[l2ds-dsbufrdresp-rtl] cycle=${debugCycle} owner=${Hexadecimal(i.U)} " +
        p"owner_r=${Hexadecimal(readReqOwner_r)} anyGrant=${anyGrant} " +
        p"datStop_r=${datStop_r} sramEn=${sram.io.en} " +
        p"rdata=0x${Hexadecimal(sram.io.rdata.data)} way=0x${Hexadecimal(readReqWay_r)}\n")
    }
  }
