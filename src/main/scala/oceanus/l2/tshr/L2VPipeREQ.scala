...
class L2VPipeREQ(clientComponents: Seq[CCHIComponent], tshrId: Int, nodeId: Int)(implicit val p: Parameters) 
  ...
  def satisfied(opcode: CCHIOpcode, state: UInt): (Bool, Bool) = (
    rxreq_opcode.is(opcode) && (io.tshr_dirResult.state >= state && io.tshr_dirResult.hit), 
    rxreq_opcode.is(opcode) && (io.tshr_dirResult.state < state || !io.tshr_dirResult.hit))
  ...
  val rxreq_client_present = (rxreq_client.asUInt & io.tshr_dirResult.clients.asUInt).orR
  val p_rxreq_client_present = (p_rxreq_client.asUInt & io.tshr_dirResult.clients.asUInt).orR

  val rxreq_peer_present = (~rxreq_client.asUInt & io.tshr_dirResult.clients.asUInt).orR
  val p_rxreq_peer_present = (~p_rxreq_client.asUInt & io.tshr_dirResult.clients.asUInt).orR
  // ----------------------------------------------------------------
  ...
  val w_s_up_rd_compdata0 = RegInit(false.B)
  val w_s_up_rd_compdata2 = RegInit(false.B)
  val s_up_rd_compdata0 = RegInit(false.B) // Scheduling upstream TXDAT CompData (DataID = 0)
  val s_up_rd_compdata2 = RegInit(false.B) // Scheduling upstream TXDAT CompData (DataID = 2)

  val w_s_up_rd_comp = RegInit(false.B)
  val s_up_rd_comp = RegInit(false.B) // Scheduling upstream TXRSP Comp for reads
  ...
  val issue_txreq_rd = issue_txreq_readshared ||
                       issue_txreq_readunique
  ...
  when (io.DnTXREQ.fire) {
    ...
    p_expcompack := io.DnTXREQ.bits.ExpCompAck.get
  }
  ...
  val issue_dn_rd_compack_hn_accepted = p_txreq_rd && 
                                        (dn_rxdat_compdata_first ||
                                         dn_rxrsp_comp ||
                                         dn_rxrsp_respsepdata)

  val issue_dn_rd_compack = p_expcompack && 
                            issue_dn_rd_compack_hn_accepted
  ...
  val meta_wr_state_readunique_UU_sat = active && p_rxreq_readunique &&
                                        sa_resp_decision &&
                                        io.tshr_dirResult.state === MetaState.US
  ...
  val tag_wr_readunique = rxreq_readunique && !io.tshr_dirResult.hit
  // --------------------------------
  ...
  val meta_wr_state_readshared_US_sat = active && p_rxreq_readshared &&
                                        sa_resp_decision &&
                                        io.tshr_dirResult.state === MetaState.UU &&
  ...
  val tag_wr_readshared = rxreq_readshared && !io.tshr_dirResult.hit
  // --------------------------------
  ...
  val ds_rd_readshared = rxreq_readshared && io.tshr_dirResult.hit

  val ds_rd = ds_rd_readunique ||
              ds_rd_readshared

  val ds_cancel_readunique = p_rxreq_readunique && (dn_rxdat_compdata_first || dn_rxrsp_respsepdata)
  ...
  val up_txrsp_resp = ParallelPriorityMux(Seq(
    (s_up_rd_comp, ParallelPriorityMux(Seq(
      ...
      (p_rxreq_readshared, Mux(io.tshr_dirResult.state === MetaState.UU || meta_wr_state_UU, CCHIResp.UC.U, CCHIResp.SC.U))
    )))
  ))
  ...
  io.UpTXRSP.bits.Way := io.tshr_dirResult.way
  ...
  val allow_up_rd_compdata0 = !w_ds_resp && !w_snpresp0 && !w_dn_rd_data0
  val allow_up_rd_compdata2 = !w_ds_resp && !w_snpresp2 && !w_dn_rd_data2
  ...
  io.UpTXDAT.bits.Way := io.tshr_dirResult.way
