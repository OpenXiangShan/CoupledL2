...
abstract class ReplacementPolicy {
  ...
  def get_next_state(state: UInt, touch_way: UInt, hit: Bool, invalid: Bool, chosen_type: Bool, req_type: UInt): UInt =
    0.U
...
class DRRIP(n_ways: Int) extends ReplacementPolicy {
  ...
  override def get_next_state(
    ...
    Mux(
      chosen_type,
      repl_BRRIP.get_next_state(state, touch_way, hit, invalid, req_type),
      repl_SRRIP.get_next_state(state, touch_way, hit, invalid, req_type)
    )
  def get_replace_way(state: UInt): UInt = {
    val RRPVVec = Wire(Vec(n_ways, UInt(2.W)))
    RRPVVec.zipWithIndex.map { case (e, i) =>
