 ...
 */
    ...
    //l1i not register-out to CORE why?
    ...
    io.l2_hint.bits.isKeyword := RegNext(ParallelPriorityMux(sliceHint, slices.map(_.io.l1Hint.bits.isKeyword))) //?? TODO -sourceId start
...
//    XSPerfAccumulate(cacheParams, "grant_data_fire", PopCount(VecInit(grant_fire)))
    XSPerfAccumulate(cacheParams, "l1d_D_stall",  (l1dGrantValids & stall).orR)
    XSPerfAccumulate(cacheParams, "l1d_grant_fail_hint", (l1dGrantValids & stall & hintValids).orR)
  }
