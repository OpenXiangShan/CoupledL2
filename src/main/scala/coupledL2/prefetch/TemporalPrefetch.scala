...
class TemporalPrefetch(implicit p: Parameters) extends TPModule {
  ...
  private val enableTP = WireInit(Constantin.createRecord("enableTP", initValue = 0.U))
  ...
  private val trainOnVaddr = WireInit(Constantin.createRecord("tp_trainOnVaddr", initValue = 1.U))
  ...
  tpDataQueue.io.enq.valid := io.tpmeta_port.resp.valid
