...
class NextLinePattern(implicit p: Parameters) extends NLModule {
  ...
  //update or insert
  s1_patternUpdateEn :=s1_trainValid && s1_trainResp.hit
  s1_patternInsertEn := s1_trainValid && !s1_trainResp.hit && s1_trainTouched 
...
class NextLinePrefetchIdeal(implicit p: Parameters) extends NLModule {
  ...
  /********test**********/
  val newPrefetcherSample  = Module(new L2NextLineSample())
  val newPrefetcherPattern = Module(new L2NextLinePattern())
  val oldPrefetcherSample  = prefetcherSample
  val oldPrefetcherPattern = prefetcherPattern
  // io.train ---> sample.train        
  newPrefetcherSample.io.train.valid := shouldTrain
  newPrefetcherSample.io.train.bits.addr := getBlockAddr(io.train.bits.addr)
  newPrefetcherSample.io.train.bits.pc := io.train.bits.pc.getOrElse(0.U)
  newPrefetcherSample.io.train.bits.timeSample := timeSampleCounter

  // Sample ---> Pattern 
  newPrefetcherSample.io.resp.ready := true.B
  newPrefetcherPattern.io.train.valid := oldPrefetcherSample.io.resp.valid
  newPrefetcherPattern.io.train.bits.pcHash := oldPrefetcherSample.io.resp.bits.pcTag
  newPrefetcherPattern.io.train.bits.touched := oldPrefetcherSample.io.resp.bits.touched
  

  //io.train ---> pattern.req 
  newPrefetcherPattern.io.req.valid := shouldQuery
  newPrefetcherPattern.io.req.bits.addr := getBlockAddr(io.train.bits.addr)
  newPrefetcherPattern.io.req.bits.pc := io.train.bits.pc.getOrElse(0.U)
  newPrefetcherPattern.io.resp.ready := true.B
