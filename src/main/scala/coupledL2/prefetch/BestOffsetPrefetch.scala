...
class StudentCoverageLearner(name: String = "")(implicit p: Parameters) extends BOPModule {
  ...
  emitOffsetDistCounters("student_best_offset", phaseBestEntry.offset, state === s_phaseEnd && phaseHasValidPool)
  emitCoverageHistogram("student_best_cov", phaseBestCov, state === s_phaseEnd && phaseHasValidPool)
  emitCoverageHistogram("student_worst_cov", phaseWorstCov, state === s_phaseEnd && phaseHasValidPool)
}
