...
class PrefetchReqBuffer(implicit p: Parameters) extends BOPModule{
  ...
  for ((e, i) <- entries.zipWithIndex){
    ...
    exp_drop(i) := s3_tlb_fire_oh(i) && s3_tlb_resp_valid && !s3_tlb_resp.miss && (
      ...
      io.tlb_req.pmp_resp.mmio || (s3_tlb_resp.pbmt === 1.U) || (s3_tlb_resp.pbmt === 2.U)
    )
  ...
  XSPerfAccumulate("tlb_excp_uncache", s3_tlb_resp_valid && (io.tlb_req.pmp_resp.mmio || (s3_tlb_resp.pbmt === 1.U) || (s3_tlb_resp.pbmt === 2.U)))
