...
//  val in      = io.in.bits
  ...
  val mergeAMask = VecInit(io.mshrInfo.map(s =>
    s.valid && s.bits.isPrefetch && sameAddr(in, s.bits) && !s.bits.willFree && !s.bits.dirHit && !s.bits.s_refill &&
      io.in.bits.fromA && (io.in.bits.opcode === AcquireBlock || io.in.bits.opcode === AcquirePerm) &&
      !s.bits.mergeA && !(io.in.bits.param === NtoT && s.bits.param === NtoB)
  )).asUInt
...
//    XSPerfAccumulate(cacheParams, "drop_prefetch", io.inPrefetch.get.valid && dup)
    if(flow){
