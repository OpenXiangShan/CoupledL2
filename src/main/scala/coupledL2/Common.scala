...
class TaskBundle(implicit p: Parameters)
  extends L2Bundle
...
class RespInfoBundle(implicit p: Parameters) extends L2Bundle with HasCHIMsgParameters {
  val opcode = UInt(3.W)
  val param = UInt(3.W)
...
class L2TlbResp(nDups: Int = 1)(implicit p: Parameters) extends L2Bundle {
  ...
  val excp = Vec(nDups, new TlbExceptions)
}
