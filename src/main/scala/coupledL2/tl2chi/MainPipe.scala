...
class MainPipe(implicit p: Parameters) extends TL2CHIL2Module with HasCHIOpcodes with HasPerfEvents {
  ...
  val metaW_wayOH = Mux(
    ...
    Mux(mshr_req_s3, UIntToOH(req_s3.way), io.dirWayOH_s3)
  )
  ...
  metaWReq_s3.bits.wayOH := Mux(resetFinish, metaW_wayOH, Fill(cacheParams.ways, true.B))
  ...
  tagWReq_s3.bits.wayOH := Mux(mshr_refill_s3 && req_s3.replTask, io.dirReplWayOH_s3, UIntToOH(req_s3.way))
  ...
  io.prefetchTrain.foreach {
    train =>
      ...
      train.bits.pfsource := Mux(req_s3.mergeA, req_s3.meta.prefetchSrc.getOrElse(PfSource.NoWhere.id.U), meta_s3.prefetchSrc.getOrElse(PfSource.NoWhere.id.U)) // TODO
