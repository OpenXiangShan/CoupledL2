...
class MainPipe(implicit p: Parameters) extends TL2CHIL2Module with HasCHIOpcodes with HasPerfEvents {
  ...
  val req_cbo_flush_s3          = sinkA_req_s3 && req_s3.opcode === CBOFlush && !(io.cmoAllWait && (meta_s3.state === INVALID))
  ...
  val isD_s3 = Mux(
    mshr_req_s3,
    mshr_cmoresp_s3 && !io.cmoAllWait || mshr_refill_s3 && !retry,
    req_s3.fromC || req_s3.fromA && !need_mshr_s3_a && !data_unready_s3_tl && req_s3.opcode =/= Hint && !io.cmoAllWait
  )
  ...
  val cmoLineDrop = io.cmoAllWait && task_s3.valid && sinkA_req_s3 && (req_s3.opcode === CBOFlush)  && (meta_s3.state === INVALID)
  val cmoLineDone = io.cmoAllWait && task_s3.valid && mshr_cmoresp_s3
