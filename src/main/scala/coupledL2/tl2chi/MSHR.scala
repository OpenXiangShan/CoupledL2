...
class MSHR(implicit p: Parameters) extends TL2CHIL2Module with HasCHIOpcodes {
  ...
  */
  ...
  // RXDAT
  val rxdatErrDataSepRespValid = WireInit(false.B)
  ifAfterIssueC {
    rxdatErrDataSepRespValid := rxdat.valid && rxdat.bits.chiOpcode.get === DataSepResp
  }
  val rxdatErrValid = rxdat.valid && (rxdat.bits.chiOpcode.get === CompData || rxdatErrDataSepRespValid)
  val rxdatNderr = rxdat.bits.respErr.getOrElse(OK) === NDERR
  val rxdatDerr = rxdat.bits.respErr.getOrElse(OK) === DERR || rxdatNderr || rxdat.bits.corrupt
  ...
  // RXRSP
  val rxrspErrRespSepDataValid = WireInit(false.B)
  ifAfterIssueC {
    rxrspErrRespSepDataValid := rxrsp.valid && rxrsp.bits.chiOpcode.get === RespSepData
  }
  val rxrspErrValid =  rxrsp.valid && (rxrspErrRespSepDataValid ||
    rxrsp.bits.chiOpcode.get === Comp && !state.w_grant && (state.s_cmoresp || state.w_releaseack))
  val rxrspNderr = rxrsp.bits.respErr.getOrElse(OK) === NDERR
  ...
  // RXDAT & RXRSP error handle
  when (rxdatErrValid || rxrspErrValid) {
    state.w_replResp := state.w_replResp || rxdatErrValid && rxdatNderr || rxrspErrRespSepDataValid && rxrspNderr
    denied := denied || rxdatErrValid && rxdatNderr || rxrspErrValid && rxrspNderr
    corrupt := corrupt || rxdatErrValid && rxdatDerr
  }
