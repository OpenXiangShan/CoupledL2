  ...
  */
  ...
  when (rxdat.valid) {
    ...
    ifAfterIssueC {
      when (rxdat.bits.chiOpcode.get === DataSepResp) {
        ...
        state.w_replResp := state.w_replResp || rxdatNderr
        gotT := rxdatIsU || rxdatIsU_PD
        gotDirty := gotDirty || rxdatIsU_PD
        gotGrantData := true.B
        denied := denied || rxdatNderr
        corrupt := corrupt || derr || rxdatNderr || rxdatCorrupt
      }
    }
  ...
  when (nestedwb_hit_match) {
    ...
    when (io.nestedwb.b_toN.get && req.fromA) {
      val nderr = rxdat.valid && rxdatNderr || rxrsp.valid && rxrspNderr || denied
      meta.state := INVALID
      dirResult.hit := false.B
      meta.clients := Fill(clientBits, false.B)
      state.w_replResp := cmo_cbo || nderr // never query replacer on CMO
