...
class MSHRCtl(implicit p: Parameters) extends TL2CHIL2Module with HasCHIOpcodes with HasPerfEvents {
  ...
  if (cacheParams.enablePerf) {
    ...
    // val stop = 100
    // val step = 5
    val acquire_period = ParallelMux(mshrs.map { case m => m.acquire_period.valid -> m.acquire_period.bits }) 
    val release_period = ParallelMux(mshrs.map { case m => m.release_period.valid -> m.release_period.bits })
    val acquire_period_en = mshrs.map(_.acquire_period.valid).reduce(_ || _)
    val release_period_en = mshrs.map(_.release_period.valid).reduce(_ || _)
