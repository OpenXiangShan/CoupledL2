...
class TXREQ(implicit p: Parameters) extends TL2CHIL2Module {
  ...
  val pipeStatus_s1_2_s5 = pipeStatus_s1_s5.tail
  ...
  val returnCredit_s1_2 = PopCount(pipeStatus_s1_s5.slice(1, 2).map(x => x.valid && !(x.bits.mshrTask && x.bits.toTXREQ)))
  ...
  val inflightCnt = PopCount(Cat(pipeStatus_s1_2_s5.map(s => s.valid && s.bits.mshrTask && s.bits.toTXREQ))) +
//    pipeStatus_s1.valid.asUInt +
    1.U - returnCredit_s1_2 + //Fix Timing: always take credit and s2 return if not take 
    queueCnt
