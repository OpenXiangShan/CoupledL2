  ...
  class CoupledL2Imp(wrapper: LazyModule) extends BaseCoupledL2Imp(wrapper)
    ...
    slices match {
      case slices: Seq[Slice] =>
        ...
        linkMonitor.io.lcrdy.req.rdy := io_lcrdy.req.rdy
        linkMonitor.io.lcrdy.rsp.rdy := io_lcrdy.rsp.rdy
        linkMonitor.io.lcrdy.dat.rdy := io_lcrdy.dat.rdy
        linkMonitor.io.lcrdy.empty := io_lcrdy.empty
        /**
