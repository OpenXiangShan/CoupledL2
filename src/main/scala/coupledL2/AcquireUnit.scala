...
class AcquireUnit(implicit p: Parameters) extends L2Module {
  ...
  a_acquire.bits.user.lift(ReqSourceKey).foreach(_ := task.reqSource)
  ...
  a_put.bits.user.lift(ReqSourceKey).foreach(_ := MemReqSource.NoWhere.id.U) //Ignore: where does Put comes from
