...
class TestTop_L2TSHRDirectoryProxy(implicit val p: Parameters) extends Module with HasL2Params {

  val module = Module(new L2TSHRDirectoryProxy(0))
...
object TestTop_L2TSHRDirectoryProxy extends App {
  ...
  (new ChiselStage).execute(args,
    ChiselGeneratorAnnotation(() => new TestTop_L2TSHRDirectoryProxy()(config)) +: TestTopFirtoolOptions())
}
