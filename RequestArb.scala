...
class RequestArb(implicit p: Parameters) extends LLCModule {
  ...
  /* Stage 1 */
  task_s1.valid := io.dirRead_s1.ready && (io.busTask_s1.valid || io.refillTask_s1.valid)
  ...
  io.busTask_s1.ready := io.dirRead_s1.ready && !io.refillTask_s1.valid
  io.refillTask_s1.ready := io.dirRead_s1.ready
